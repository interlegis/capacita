...
def necessidade_query(area,nivel,plano,turno, qtd_servidor):
    ...
    if (area and nivel and (not plano) and turno and qtd_servidor):
        necessidades = Necessidade.objects.filter(cod_sub_area_conhecimento_id__cod_area_conhecimento = area, cod_nivel = nivel, cod_turno = turno, qtd_servidor__range = (qtd_servidor - 2, qtd_servidor + 2))
    if (area and (not nivel) and plano and turno and qtd_servidor): 
        necessidades = Necessidade.objects.filter(cod_sub_area_conhecimento_id__cod_area_conhecimento = area, cod_plano_capacitacao = plano, cod_turno = turno, qtd_servidor__range = (qtd_servidor - 2, qtd_servidor + 2))
    if ((not area) and nivel and plano and turno and qtd_servidor):
        necessidades = Necessidade.objects.filter(cod_plano_capacitacao = plano, cod_nivel = nivel, cod_turno = turno, qtd_servidor__range = (qtd_servidor - 2, qtd_servidor + 2))
    if (area and (not nivel) and (not plano) and turno and qtd_servidor):
        necessidades = Necessidade.objects.filter(cod_sub_area_conhecimento_id__cod_area_conhecimento = area, cod_turno = turno, qtd_servidor__range = (qtd_servidor - 2, qtd_servidor + 2))
    if ((not area) and nivel and (not plano) and turno and qtd_servidor):
         necessidades = Necessidade.objects.filter(cod_nivel = nivel, cod_turno = turno, qtd_servidor__range = (qtd_servidor - 2, qtd_servidor + 2))
    if ((not area) and (not nivel) and plano and turno and qtd_servidor):
         necessidades = Necessidade.objects.filter(cod_plano_capacitacao = plano, cod_turno = turno, qtd_servidor__range = (qtd_servidor - 2, qtd_servidor + 2))
    if ((not area) and (not nivel) and (not plano) and turno and qtd_servidor):
         necessidades = Necessidade.objects.filter(cod_turno = turno, qtd_servidor__range = (qtd_servidor - 2, qtd_servidor + 2))
